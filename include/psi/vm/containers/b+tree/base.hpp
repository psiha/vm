...
public:
    ...
    // loaded before a node's keys can be addressed, and a small-node tree
    // visits more nodes.  Those figures were taken with that load on every node
    // a search visits; an inner node, which never carries a gap, now addresses
    // its keys and children from a compile-time 0 (node_header::live_start),
    // which leaves the load on the leaf alone.  Both the cost and the benefit
    // scale with node size, so this could be decided per node geometry rather
    // than globally.
...
#   endif
        ...
        // carries a gap (verify_gap asserts the stored field stays 0), so its
        // keys and children are addressed from the node's address alone.  A
        ...
        // arrived, and a descent pays that at every inner level rather than
        // once, at the leaf.  Same type as the field, so every index computed
        // from it promotes exactly as it did from the field.
        [[ gnu::pure ]] constexpr std::uint8_t live_start( this auto const & self ) noexcept
        {
            if constexpr ( requires{ self.children_; } ) return 0;
