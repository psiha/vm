...
private:
    ...
    {
        // The binary arm is node_binary_lower_bound (base.hpp): for a comparator
        // that reads only the keys, a branchless halving that prefetches both
        // candidate next probes, unless PSI_VM_BT_PREFETCHING_BINARY_SEARCH says
        // otherwise; for any other, plain binary_lower_bound.
...
#       else
            use_linear_search_for_sorted_array<Comparator, Key, maximum_values>
                ...
                : node_binary_lower_bound<Comparator, Key>( &keys[ 0 ], &keys[ num_vals ], value, make_trivially_copyable_predicate( comp ) )
#       endif
        };
...
#       else
            use_linear_search_for_sorted_array<Comparator, Key, maximum_values>
                ...
                : node_binary_upper_bound<Comparator, Key>( &keys[ 0 ], &keys[ num_vals ], value, make_trivially_copyable_predicate( comp ) )
#       endif
        };
